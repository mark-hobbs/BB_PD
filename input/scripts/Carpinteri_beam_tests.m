...
% =========================================================================
% Create input files
% Cracking and crackling in concrete-like materials: A dynamic energy
% balance - Carpinteri et al., 2016
% =========================================================================
...
member.LENGTH = 0.84;      % x-axis (m) 
member.WIDTH = 0.1;        % y-axis (m) 
member.DEPTH = 0.1;        % z-axis (m)
...
supportRadius = 20 * DX;    % (5 * DX = 25mm)
searchRadius = 40.1 * DX;   % (10.1 * DX = 50.5mm)
supportCentreX = [ (DX * (0.02/DX)) , DX * (0.82/DX) + DX ];
...
penetratorRadius = 10 * DX;     % (10 * DX = 50mm)
searchRadius = 15.1 * DX;       % (15.1 * DX = 75.5mm)
...
[BONDLIST, UNDEFORMEDLENGTH, nFAMILYMEMBERS, NODEFAMILYPOINTERS, NODEFAMILY] = buildnotch(undeformedCoordinates, BONDLIST, UNDEFORMEDLENGTH, DX, 84.5, 10.1);
...
simulation.appliedDisplacement = -1E-3;      % Applied displacement (m)
simulation.referenceNode = 84;               % Define a reference point/node for measuring deflections
simulation.CMOD = [40 128];                  % Define reference nodes for measuring CMOD
